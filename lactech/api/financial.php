...
try {
    ...
    if ($method === 'GET') {
        ...
        switch ($action) {
            ...
            case 'get_by_id':
                ...
                $data = $db->getFinancialRecordById($id);
                sendResponse($data);
                break;
    ...
    // POST: Adicionar novo registro financeiro
    if ($method === 'POST') {
        ...
        if ($action === 'insert') {
            unset($input['action']);
            $data = $db->addFinancialRecord($input);
            sendResponse($data);
        } else {
