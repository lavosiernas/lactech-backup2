...
try {
    ...
    try {
        ...
        $stmt = $prepareSiteFilter($sqlToday);
        $todayStats = $stmt ? $stmt->fetch(PDO::FETCH_ASSOC) ?: [] : [];
    } catch (PDOException $e) {
    ...
    try {
        ...
        $stmt = $prepareSiteFilter($sql24h);
        $last24hStats = $stmt ? $stmt->fetch(PDO::FETCH_ASSOC) ?: [] : [];
    } catch (PDOException $e) {
    ...
    try {
        ...
        $stmt = $prepareSiteFilter($sqlYesterday);
        $yesterdayStats = $stmt ? $stmt->fetch(PDO::FETCH_ASSOC) ?: [] : [];
    } catch (PDOException $e) {
    ...
    try {
        $sqlRecent = "SELECT * FROM safenode_security_logs WHERE 1=1 ORDER BY created_at DESC LIMIT 10";
        $stmt = $prepareSiteFilter($sqlRecent);
    ...
    $topBlockedIPs = [];
    try {
        $sqlTopIPs = "SELECT ip_address, COUNT(*) AS block_count, MAX(created_at) AS last_blocked, 
        ...
        'data' => [
            ...
            'event_logs' => array_map(function($log) {
                ...
                return [
                    ...
                    'threat_type' => $log['threat_type'] ?? null,
                    'threat_score' => (int)($log['threat_score'] ?? 0),
