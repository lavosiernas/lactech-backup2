...
<body x-data="{ notificationsOpen: false, sidebarOpen: false }" class="h-full overflow-hidden flex">
    ...
    <aside class="sidebar w-72 h-full flex-shrink-0 flex flex-col hidden lg:flex">
        ...
        <nav class="flex-1 p-5 space-y-2 overflow-y-auto">
            ...
            <a href="attacked-targets.php" class="nav-item active">
    ...
    <aside x-show="sidebarOpen"
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto overflow-x-hidden">
            ...
            <a href="attacked-targets.php" class="nav-item active" @click="sidebarOpen = false">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden bg-dark-950">
        ...
        <div class="flex-1 overflow-y-auto p-8">
            <div id="targets-content" class="space-y-6">
                <div class="text-center py-12 text-zinc-500">
                    <i data-lucide="loader" class="w-12 h-12 mx-auto mb-4 animate-spin"></i>
                    <p class="text-sm">Carregando alvos atacados...</p>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        async function fetchAttackedTargets() {
            try {
                ...
                if (data.success) {
                    targetsData = data.data?.analytics?.attacked_targets || [];
                    console.log('Attacked Targets Data:', targetsData);
                    updateTargetsPage();
                } else {
                    throw new Error(data.error || 'Erro desconhecido');
                }
            } catch (error) {
                console.error('Erro ao buscar alvos atacados:', error);
                document.getElementById('targets-content').innerHTML = `
                    <div class="table-card p-8 text-center">
                        ...
                        <button onclick="fetchAttackedTargets()" class="mt-4 px-4 py-2 btn-primary text-sm">Tentar novamente</button>
                    </div>
                `;
                lucide.createIcons();
            }
        ...
        function updateTargetsPage() {
            ...
            if (targets.length === 0) {
                container.innerHTML = `
                    <div class="table-card p-8 text-center">
                        ...
                        <p class="text-white font-bold mb-2">Nenhum alvo atacado recentemente</p>
                        <p class="text-zinc-500 text-sm">Todas as rotas estão seguras</p>
                        <p class="text-zinc-600 text-xs mt-4">Alvos atacados aparecerão aqui quando detectados pelo sistema</p>
                    </div>
                `;
                lucide.createIcons();
                return;
            ...
            container.innerHTML = `
                <div class="table-card p-6 mb-6">
                    <div class="flex items-center justify-between">
                        <h2 class="text-xl font-bold text-white">${targets.length} Alvo${targets.length !== 1 ? 's' : ''} Identificado${targets.length !== 1 ? 's' : ''}</h2>
                            ...
                            <div class="stat-card">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-3 mb-2">
                                            ...
                                            <p class="text-lg font-mono font-bold text-white truncate">${target.request_uri}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
                                    <div class="p-3 rounded-lg bg-dark-700 border border-white/5">
                                        ...
                                        <p class="text-lg font-bold text-white">${target.attack_count || 0}</p>
                                    </div>
                                    <div class="p-3 rounded-lg bg-dark-700 border border-white/5">
                                        <p class="text-xs text-zinc-400 mb-1">Atacantes Únicos</p>
                                        <p class="text-lg font-bold text-white">${target.unique_attackers || 0}</p>
                                    </div>
                                    <div class="p-3 rounded-lg bg-dark-700 border border-white/5">
                                        <p class="text-xs text-zinc-400 mb-1">Tipos de Ameaça</p>
                                        <p class="text-lg font-bold text-white">${target.threat_types_count || 0}</p>
                                    </div>
                                    <div class="p-3 rounded-lg bg-dark-700 border border-white/5">
                                        <p class="text-xs text-zinc-400 mb-1">Threat Score Médio</p>
                                        <p class="text-lg font-bold text-white">${(target.avg_threat_score || 0).toFixed(1)}</p>
                                    </div>
                                </div>
                            </div>
                        `;
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', function() {
                ...
                fetchAttackedTargets();
                setInterval(fetchAttackedTargets, 10000);
            });
        } else {
            lucide.createIcons();
