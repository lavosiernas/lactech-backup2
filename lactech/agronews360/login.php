...
<body>
    <div class="login-container min-h-screen flex items-center justify-center p-4">
        <div class="w-full max-w-md">
            ...
            <div class="login-card rounded-2xl p-8 md:p-10">
                ...
                <!-- Login com Google (AgroNews - Principal) -->
                <button id="googleLoginBtnAgronews" class="google-btn w-full mb-5 px-6 py-3.5 rounded-xl flex items-center justify-center space-x-3 font-semibold text-gray-700 text-[15px]">
                ...
                <!-- Divisor -->
                <div class="divider my-6" id="dividerBeforeLactech">
                ...
                <!-- Login com Email/Senha e Google Lactech (Oculto por padrão) -->
                <div id="lactechLoginSection" class="hidden" style="display: none;">
                    ...
                    <!-- Login com Google (Lactech) -->
                    <button id="googleLoginBtnLactech" class="google-btn w-full mb-4 px-6 py-3.5 rounded-xl flex items-center justify-center space-x-3 font-semibold text-gray-700 text-[15px]">
                        <svg class="w-5 h-5" viewBox="0 0 24 24">
                            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                        ...
                        <span>Entrar com Google (Lactech)</span>
                    </button>
                </div>
    ...
    <script>
        ...
        const lactechLoginSection = document.getElementById('lactechLoginSection');
        const googleLoginBtnAgronews = document.getElementById('googleLoginBtnAgronews');
        const dividerBeforeLactech = document.getElementById('dividerBeforeLactech');
        
        // Verificar se todos os elementos existem
        if (!toggleLactechBtn || !lactechLoginSection || !googleLoginBtnAgronews || !dividerBeforeLactech) {
            console.error('Erro: Elementos do toggle Lactech não encontrados');
        }
        ...
        if (toggleLactechBtn) {
            ...
            if (lactechFormVisible) {
                // Ocultar botão Google do AgroNews e divisor
                googleLoginBtnAgronews.style.display = 'none';
                dividerBeforeLactech.style.display = 'none';
                
                // Mostrar seção do Lactech
                lactechLoginSection.classList.remove('hidden');
                lactechLoginSection.style.display = 'block';
                lactechLoginSection.style.opacity = '0';
                lactechLoginSection.style.transform = 'translateY(8px)';
                lactechLoginSection.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
                
                // Forçar reflow para garantir que a animação funcione
                void lactechLoginSection.offsetHeight;
                
                // Animar entrada
                requestAnimationFrame(() => {
                    lactechLoginSection.style.opacity = '1';
                    lactechLoginSection.style.transform = 'translateY(0)';
                });
                
                toggleLactechText.textContent = 'Ocultar opção Lactech';
                ...
                // Ocultar seção do Lactech
                lactechLoginSection.style.opacity = '0';
                lactechLoginSection.style.transform = 'translateY(-10px)';
                
                setTimeout(() => {
                    lactechLoginSection.classList.add('hidden');
                    lactechLoginSection.style.display = 'none';
                    lactechLoginSection.style.opacity = '';
                    lactechLoginSection.style.transform = '';
                    lactechLoginSection.style.transition = '';
                    
                    // Mostrar botão Google do AgroNews e divisor novamente
                    googleLoginBtnAgronews.style.display = 'flex';
                    dividerBeforeLactech.style.display = 'block';
                }, 300);
        ...
        // Login com Google (AgroNews)
        document.getElementById('googleLoginBtnAgronews').addEventListener('click', async function() {
            const btn = this;
            btn.disabled = true;
            btn.style.opacity = '0.7';
            
            try {
                const response = await fetch('api/auth.php?action=get_google_auth_url&type=agronews');
                const result = await response.json();
                ...
                console.error('Erro:', error);
                showError('Erro ao conectar com Google. Verifique se o AgroNews está configurado.');
                btn.disabled = false;
                btn.style.opacity = '1';
            }
        });
        ...
        // Login com Google (Lactech)
        document.getElementById('googleLoginBtnLactech').addEventListener('click', async function() {
            const btn = this;
            btn.disabled = true;
            btn.style.opacity = '0.7';
            
            try {
                const response = await fetch('api/auth.php?action=get_google_auth_url&type=lactech');
                const result = await response.json();
        ...
        function showError(message) {
            ...
            if (toggleBtn) {
                ...
                document.querySelector('.login-card').insertBefore(errorDiv, document.querySelector('.login-card').firstChild);
            }
