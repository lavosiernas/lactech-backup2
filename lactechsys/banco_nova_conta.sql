...
-- Função para obter perfil do usuário (SEM PARÂMETROS)
...
-- Políticas para tabela users (VERSÃO 2 - SIMPLES SEM RECURSÃO)
...
BEGIN
    ...
    RAISE NOTICE '🔧 Funções RPC criadas: check_farm_exists, get_user_profile (SEM PARÂMETROS), create_farm_with_owner, get_farm_users, get_farm_stats, get_quality_stats, get_volume_stats, check_user_exists, create_initial_farm, create_initial_user, complete_farm_setup, register_quality_test';
    RAISE NOTICE '🔒 RLS configurado para todas as tabelas (users com políticas simples sem recursão)';
    ...
    RAISE NOTICE '🔧 CORREÇÕES APLICADAS: get_user_profile() sem parâmetros, RLS users simplificado';
END $$;
