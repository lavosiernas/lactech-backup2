...
async function updateStatistics() {
    try {
        ...
        if (stats.monthProduction !== undefined) {
            const el = document.getElementById('monthProduction');
            if (el) el.textContent = stats.monthProduction;
        }
...
async function initializePage() {
    try {
        log('Iniciando sistema MySQL...', 'info');
        ...
        log('Sistema MySQL inicializado com sucesso!', 'success');
...
document.addEventListener('DOMContentLoaded', async function() {
    log('DOM carregado, inicializando sistema MySQL...', 'info');
...
function setupFormListeners() {
    ...
    // Formulário de volume individual
    const individualVolumeForm = document.getElementById('individualVolumeForm');
    if (individualVolumeForm) {
        individualVolumeForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            try {
                await createVolumeRecord(data);
                e.target.reset();
                closeModal('individualVolumeModal');
    ...
    if (qualityTestForm) {
        qualityTestForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData.entries());
            try {
                await createQualityTest(data);
                e.target.reset();
                closeModal('qualityTestModal');
...
log('Gerente MySQL carregado com sucesso!', 'success');
