...
<head>
    ...
    <script>
        ...
        function applyChartTheme() {
            try {
                ...
                const charts = [window.volumeChart, window.dashboardWeeklyChart, window.temperatureChart, window.qualityChart, window.weeklyVolumeChart, window.dailyVolumeChart, window.qualityTrendChart, window.qualityDistributionChart, window.paymentsChart, window.weeklySummaryChart, window.monthlyVolumeChart].filter(Boolean);
    ...
    <style>
        ...
        .btn-shimmer::before { content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%; background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.4), transparent); transition: left 0.6s ease-in-out; }
        .btn-shimmer:hover::before { left: 100%; }
        .glass-effect { backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); }
        .shadow-red-glow { box-shadow: 0 10px 40px rgba(239, 68, 68, 0.3); }
        .shadow-green-glow { box-shadow: 0 10px 40px rgba(34, 197, 94, 0.3); }
        .shadow-purple-glow { box-shadow: 0 10px 40px rgba(147, 51, 234, 0.3); }
        .floating-particles::before, .floating-particles::after { content: ''; position: absolute; width: 4px; height: 4px; background: rgba(255, 255, 255, 0.3); border-radius: 50%; animation: float 3s ease-in-out infinite; }
        .floating-particles::before { top: 20%; right: 15%; animation-delay: -1s; }
        .floating-particles::after { bottom: 25%; left: 10%; animation-delay: -2s; }
    </style>
    ...
    <script>
        ...
        (function() {
            ...
            destroyModals();
            // Removed setInterval(destroyModals, 100) - was causing modal interference
            window.deleteUser = function() { return false; };
            window.closePhotoChoiceModal = function() { const photoModal = document.getElementById('photoChoiceModal'); if (photoModal) photoModal.remove(); };
            window.choosePhotoSource = function() { const photoModal = document.getElementById('photoChoiceModal'); if (photoModal) photoModal.remove(); };
        })();
    ...
    <style>
        #photoChoiceModal.show #modalContent { transform: scale(1); opacity: 1; } 
        button:focus { outline: none; } 
        .focus-visible:focus { --tw-ring-offset-shadow: var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color); --tw-ring-shadow: var(--tw-ring-inset) 0 0 0 calc(2px + var(--tw-ring-offset-width)) var(--tw-ring-color); box-shadow: var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000); --tw-ring-color: rgba(134, 209, 134, 0.5); --tw-ring-offset-width: 2px; } 
        .bg-forest-100 { background-color: #f0f9f0; } 
        :root { --forest-300: #86d186; }
    </style>
    ...
    <script>
        ...
        async function initializePage() {
            try {
                ...
                const { data: userData, error: userError } = await supabase.from('users').select('is_active').eq('id', user.id).single();
                ...
                try { await setFarmName(); } catch (error) { console.error('Error setting farm name:', error); }
                try { await setManagerName(); } catch (error) { console.error('Error setting manager name:', error); }
                try { await loadUserProfile(); } catch (error) { console.error('Error loading user profile:', error); }
        ...
        async function getManagerName() {
            try {
                ...
                const { data, error } = await supabase.from('users').select('name').eq('email', user.email).single();
        ...
        async function setManagerName() {
            ...
            const elements = ['profileName', 'profileFullName'];
        ...
        async function loadVolumeData() {
            try {
                ...
                const { data: volumeUserData, error: userError } = await supabase.from('users').select('farm_id').eq('email', user.email).single();
                ...
                const { data: todayData, error: todayError } = await supabase.from('milk_production').select('volume_liters').eq('farm_id', volumeUserData.farm_id).gte('production_date', today).lte('production_date', today);
                ...
                const { data: weekData, error: weekError } = await supabase.from('milk_production').select('volume_liters').eq('farm_id', volumeUserData.farm_id).gte('production_date', weekAgo.toISOString().split('T')[0]);
                ...
                const weekAvg = weekData?.length > 0 ? weekData.reduce((sum, record) => sum + (record.volume_liters || 0), 0) / weekData.length : 0;
                ...
                const { data: lastCollectionData, error: lastCollectionError } = await supabase.from('milk_production').select('created_at').eq('farm_id', volumeUserData.farm_id).order('created_at', { ascending: false }).limit(1);
                ...
                if (!lastCollectionError && lastCollectionData && lastCollectionData.length > 0) {
                    ...
                    const dateStr = lastCollectionTime.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                    const timeStr = lastCollectionTime.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
        ...
        async function loadQualityData() {
            try {
                ...
                const { data: qualityUserData, error: userError } = await supabase.from('users').select('farm_id').eq('email', user.email).single();
                ...
                const { data: qualityData, error } = await supabase.from('quality_tests').select('fat_percentage, protein_percentage, scc, cbt, test_date').eq('farm_id', qualityUserData.farm_id).not('fat_percentage', 'is', null).not('protein_percentage', 'is', null).order('test_date', { ascending: false }).limit(30);
                ...
                if (qualityData && qualityData.length > 0) {
                    const validQualityData = qualityData.filter(record => record.fat_percentage && record.protein_percentage && !isNaN(record.fat_percentage) && !isNaN(record.protein_percentage));
                    ...
                    validQualityData.forEach(record => {
                        if (record.fat_percentage && record.protein_percentage) {
                            ...
                            if (record.fat_percentage >= 3.0 && record.fat_percentage <= 4.5) {
                                ...
                                fatScore = Math.max(0, (record.fat_percentage / 2.5) * 70);
                            }
                            ...
                            if (record.protein_percentage >= 3.0 && record.protein_percentage <= 3.8) {
                                ...
                                proteinScore = Math.max(0, (record.protein_percentage / 2.7) * 70);
                            }
                    ...
                    document.getElementById('fatContent').textContent = avgFat.toFixed(2) + '%';
                    document.getElementById('proteinContent').textContent = avgProtein.toFixed(2) + '%';
                    document.getElementById('sccCount').textContent = Math.round(avgSCC).toLocaleString();
                    document.getElementById('tbc').textContent = Math.round(avgTBC).toLocaleString();
                    document.getElementById('qualityAverage').textContent = avgQuality.toFixed(1) + '%';
