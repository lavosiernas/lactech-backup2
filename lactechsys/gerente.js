...
// Supabase completamente removido - usando apenas MySQL via apiRequest
...
// MySQL: Fazenda fixa - Lagoa do Mato
const existingFarm = { id: 1 }; // ID fixo da fazenda

// MySQL: Fazenda fixa - Lagoa do Mato
const farmId = 1;

// MySQL: Usuário já deve estar criado via API
// Não precisa criar automaticamente aqui
} catch (error) {
throw error; // Re-throw the error so it can be handled by the caller
}
}

// Function to get farm name from session
...
async function getPrimaryUserAccount(email) {
    try {
        // MySQL: Buscar usuário via API
        const result = await apiRequest(`users/get_by_email.php?email=${encodeURIComponent(email)}`);
...
// Function to get manager name from session
...
try {
    const data = await apiRequest('users/get.php', {
        method: 'POST',
        body: { user_id: user.id }
    });
    return data?.name || 'Gerente';
} catch (error) {
    return user.name || 'Gerente';
}
} catch (error) {
...
// MySQL: Buscar dados via API
const userData = await apiRequest('users/get.php'
    .from('users')
...
try {
    const volumeData = await apiRequest('volume/list.php', {
        method: 'POST',
        body: { 
...
try {
    ...
    const qualityData = await apiRequest('quality/list.php', {
        method: 'POST',
        body: { 
            farm_id: userData.farm_id,
...
try {
    const financialData = await apiRequest('financial/list.php', {
        method: 'POST',
        body: { 
            farm_id: userData.farm_id,
            type: 'expense',
...
// Load users data
try {
    const usersData = await apiRequest('users/list.php', {
        method: 'POST',
        body: { 
            farm_id: userData.farm_id,
            is_active: true
...
const volumeUserData = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: user.id }
});
...
const today = new Date().toISOString().split('T')[0];
const todayData = await apiRequest('volume/list.php', {
    method: 'POST',
    body: { 
        farm_id: volumeUserData.farm_id,
        start_date: today,
        end_date: today
    }
});
...
const weekData = await apiRequest('volume/list.php', {
    method: 'POST',
    body: { 
        farm_id: volumeUserData.farm_id,
...
const lastCollectionData = await apiRequest('volume/last.php', {
    method: 'POST',
    body: { farm_id: volumeUserData.farm_id }
});

if (lastCollectionData && lastCollectionData.length > 0) {
...
const qualityUserData = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: user.id }
});
...
// MySQL: Buscar qualidade via API
const qualityData = await apiRequest('quality/list.php', {
    method: 'POST',
    body: { 
        farm_id: qualityUserData.farm_id,
        limit: 30
    }
});
...
const salesData = await apiRequest('financial/list.php', {
    method: 'POST',
    body: { 
        farm_id: userData.farm_id,
        type: 'income'
...
let usersUserData = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: user.id }
});
...
const usersData = await apiRequest('users/list.php', {
    method: 'POST',
    body: { farm_id: usersUserData.farm_id }
});
...
const today = new Date().toISOString().split('T')[0];
// MySQL: Buscar volume via API
const volumeData = await apiRequest('volume/daily.php', {
    method: 'POST',
    body: { 
        farm_id: userData.farm_id,
        date: today
...
// Obter email do usuário atual do localStorage/sessionStorage
...
const user = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: userId }
});
...
const currentUser = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: userId }
});

// Now update the user status
await apiRequest('users/update.php', {
    method: 'POST',
    body: { 
        user_id: userId,
        is_active: newStatus
...
// Get farm users
const usersData = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: user.id }
});

// Get all users from the same farm
const farmUsers = await apiRequest('users/list.php', {
    method: 'POST',
    body: { farm_id: usersData.farm_id }
});
...
await apiRequest('users/delete.php', {
    method: 'POST',
    body: { user_id: userId }
});
...
try {
    ...
    const result = await apiRequest('users/restore.php', {
    method: 'POST',
    body: {
...
// Nota: Senhas são gerenciadas pela API MySQL
// Se uma nova senha foi fornecida, ela deve ser atualizada via MySQL API
if (password && password.trim() !== '') {
    // TODO: Implementar atualização de senha via API se necessário
...
const volumeRecords = await apiRequest('volume/list.php', {
    method: 'POST',
    body: { 
        farm_id: userData.farm_id,
        limit: 20
...
const qualityTests = await apiRequest('quality/list.php', {
    method: 'POST',
    body: { 
        farm_id: qualityUserData.farm_id,
        limit: 20
...
// Delete the record
await apiRequest('volume/delete.php', {
    method: 'POST',
...
// Delete the test
await apiRequest('quality/delete.php', {
    method: 'POST',
    body: { test_id: testId }
});
...
const userProfile = await apiRequest('users/get.php', {
    method: 'POST',
    body: { user_id: user.id }
});
...
// Get temperature data from volume records
const temperatureData = await apiRequest('volume/temperature.php', {
    method: 'POST',
    body: { 
        farm_id: userData.farm_id,
        limit: 7
...
const activities = await apiRequest('activities/recent.php', {
    method: 'POST',
    body: { 
        farm_id: farmId,
        limit: 5
...
// DESABILITADO: Real-time não disponível com MySQL - use refresh manual ou polling se necessário
async function setupRealtimeUpdates() {
console.log('ℹ️ Atualizações em tempo real desabilitadas (MySQL não suporta nativamente)');
// Para implementar atualizações em tempo real com MySQL, considere:
// 1. Polling periódico com setInterval
// 2. WebSocket server separado
...
const volumeData = await apiRequest('volume/list.php', {
    method: 'POST',
    body: { 
        farm_id: parsedUserData.farm_id,
        start_date: new Date().toISOString().split('T')[0]
    }
});
...
// Update user table in database via API
await apiRequest('users/update.php', {
    method: 'POST',
    body: {
        user_id: user.id,
...
const managerData = await apiRequest('users/get.php', {
    method: 'POST',
...
if (navigator.onLine && window.offlineSyncManager) {
    ...
    try {
        await apiRequest('volume/create.php', {
            method: 'POST',
            body: insertData
        });
...
await apiRequest('quality/create.php', {
    method: 'POST',
    body: {
...
await apiRequest('financial/create.php', {
    method: 'POST',
    body: {
        ...paymentData,
        farm_id: managerData.farm_id,
...
// MySQL via apiRequest
// Get farm name
const farmData = await apiRequest('farms/get.php', {
    method: 'POST',
    body: { farm_id: farmId }
});
...
while (attempts < maxAttempts) {
    ...
    const existingUser = await apiRequest('users/check-email.php', {
        method: 'POST',
...
// MySQL: Criar usuário via API
const authData = await apiRequest('users/create.php', {
    method: 'POST',
    body: {
        email: email,
        password: password,
...
const result = await apiRequest('users/create.php', {
    method: 'POST',
    body: {
...
if (userData.role === 'funcionario' && profilePhotoFile && profilePhotoFile.size > 0) {
    ...
    try {
        ...
        const profilePhotoUrl = await uploadProfilePhoto(profilePhotoFile, authData.id);
...
await apiRequest('users/update_settings.php', {
    method: 'POST',
    body: {
...
if (!userData?.farm_id) return '001';

// Contar usuários existentes na mesma fazenda
const existingUsers = await apiRequest('users/list.php', {
    method: 'POST',
    body: { farm_id: userData.farm_id }
});
...
if (!managerData?.farm_id) {
    console.error('DEBUG: Fazenda não encontrada');
...
// MySQL: Upload de arquivo via API
const result = await apiRequest('files/upload.php', {
    method: 'POST',
    body: {
    .from('profile-photos')
...
// MySQL: URL do arquivo via API
const publicUrl = await apiRequest('files/get_url.php', {
    .from('profile-photos')
...
const allUsers = await apiRequest('users/list.php', {
    method: 'POST',
    body: { farm_id: userData.farm_id }
});
...
// MySQL via apiRequest
clearUserSession(); // MySQL: Limpar sessão local
...
try {
    const relationData = await apiRequest('secondary-accounts/get-relation.php', {
        method: 'POST',
        body: { primary_account_id: user.id }
    });
...
if (!secondaryAccountRelation) {
    try {
        ...
        const secondaryAccount = await apiRequest('users/get-by-email.php', {
            method: 'POST',
            body: { email: `${userData.email}.sec` }
        });
        ...
        if (secondaryAccount) {
            ...
            try {
                await apiRequest('secondary-accounts/create-relation.php', {
                    method: 'POST',
                    body: {
    ...
    try {
        ...
        const secondaryAccount = await apiRequest('users/get.php', {
            method: 'POST',
            body: { user_id: secondaryAccountRelation.secondary_account_id }
        });
...
const existingAccount = await apiRequest('users/check-email.php', {
    method: 'POST',
    body: { 
...
if (existingAccount) {
    ...
    secondaryAccount = await apiRequest('users/update.php', {
        method: 'POST',
        body: {
    ...
    const existingRelation = await apiRequest('secondary-accounts/get-relation.php', {
        method: 'POST',
        body: { primary_account_id: user.id, secondary_account_id: secondaryAccount.id }
    });
    ...
    const existingUsers = await apiRequest('users/check-email.php', {
        method: 'POST',
        body: { 
            email: secondaryEmail,
            farm_id: userData.farm_id
...
// Obter URL pública
// MySQL: Obter URL do arquivo
const urlData = await apiRequest('files/get_url.php', {
    .from('profile-photos')
...
// Nota: O usuário precisará usar a função de recuperação de senha do sistema
...
// MySQL: Buscar dados do usuário
const managerData = { farm_id: 1, name: user.name, whatsapp: user.whatsapp };
...
// Upload da foto do gerente para o servidor
async function uploadManagerProfilePhoto(file, userId) {
try {
// MySQL via apiRequest
...
async function removeManagerPhoto() {
    try {
        ...
        if (userData && userData.profile_photo_url) {
            ...
            if (photoPath) {
                await apiRequest('files/upload.php', {
    method: 'POST',
    body: {
                    .from('profile-photos')
                    .remove([photoPath]);
...
async function sendFileMessage(file) {
    ...
    try {
        ...
        const { data: uploadData, error: uploadError } = await apiRequest('files/upload.php', {
    method: 'POST',
    body: {
            .from('chat-files')
            .upload(filePath, file);
