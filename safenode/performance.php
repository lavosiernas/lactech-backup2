...
<body class="h-full" x-data="{ sidebarOpen: false }">
            ...
            <div class="flex-1 overflow-y-auto p-8">
                <div id="performance-content" class="space-y-6">
                    <div class="glass rounded-2xl p-12 text-center">
                        ...
                        <p class="text-sm text-gray-700 dark:text-zinc-500">Carregando dados de performance...</p>
                    </div>
                </div>
            </div>
        </main>
    ...
    <script>
        let performanceData = { stats: null, endpoints: null, recommendations: null };

        async function fetchPerformanceData() {
            try {
                const [statsRes, endpointsRes, recommendationsRes] = await Promise.all([
                    fetch('api/performance-stats.php?timeframe=24h'),
                    fetch('api/performance-endpoints.php?timeframe=24h&limit=20'),
                    fetch('api/performance-recommendations.php?timeframe=7d')
                ]);
                
                if (!statsRes.ok || !endpointsRes.ok || !recommendationsRes.ok) {
                    throw new Error('Erro ao buscar dados de performance');
                }
                
                const [statsData, endpointsData, recommendationsData] = await Promise.all([
                    statsRes.json(),
                    endpointsRes.json(),
                    recommendationsRes.json()
                ]);
                
                if (statsData.success) performanceData.stats = statsData.data;
                if (endpointsData.success) performanceData.endpoints = endpointsData.data;
                if (recommendationsData.success) performanceData.recommendations = recommendationsData.data;
                ...
                document.getElementById('performance-content').innerHTML = `
                    <div class="glass rounded-2xl p-8 text-center">
                        ...
                        <button onclick="fetchPerformanceData()" class="px-4 py-2 bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white rounded-xl hover:bg-gray-200 dark:hover:bg-white/20 transition-colors text-sm">
                            Tentar novamente
                        </button>
                    </div>
                `;
        ...
        function updatePerformancePage() {
            ...
            const endpoints = (performanceData.endpoints?.endpoints || []).slice(0, 20);
            const recommendations = (performanceData.recommendations?.recommendations || []).slice(0, 10);
            
            // Verificar se a tabela não foi criada (mensagem da API)
            if (stats.message && stats.message.includes('Tabela de performance ainda não foi criada')) {
                container.innerHTML = `
                    <div class="table-card p-8 text-center">
                        <i data-lucide="info" class="w-12 h-12 mx-auto mb-4 text-blue-400"></i>
                        <p class="text-gray-900 dark:text-white font-bold mb-2">Nenhum dado de performance</p>
                        <p class="text-gray-600 dark:text-zinc-500 text-sm">Execute o SQL da Fase 2 para ativar o monitoramento de performance</p>
                    </div>
                `;
                lucide.createIcons();
                return;
            ...
            // Se a tabela existe mas não há dados ainda
            if (!stats || stats.total_requests === 0 || !stats.total_requests) {
                container.innerHTML = `
                    <div class="table-card p-8 text-center">
                        <i data-lucide="check-circle" class="w-12 h-12 mx-auto mb-4 text-green-400"></i>
                        <p class="text-gray-900 dark:text-white font-bold mb-2">Monitoramento Ativo</p>
                        <p class="text-gray-600 dark:text-zinc-500 text-sm mb-4">O monitoramento de performance está funcionando. Os dados aparecerão aqui quando houver requisições lentas (> 500ms ou > 1MB).</p>
                        <div class="glass rounded-xl p-4 text-left max-w-md mx-auto mt-4">
                            <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2"><strong>Como funciona:</strong></p>
                            <ul class="text-xs text-gray-600 dark:text-zinc-400 space-y-1 list-disc list-inside">
                                <li>O sistema monitora todas as requisições automaticamente</li>
                                <li>Registra apenas requisições lentas (> 500ms ou > 1MB)</li>
                                <li>Isso mantém o foco nos problemas de performance</li>
                            </ul>
                        </div>
                    </div>
                `;
            ...
            const performanceScore = stats.performance_score || 0;
            const scoreColor = performanceScore >= 80 ? 'green' : performanceScore >= 60 ? 'orange' : 'red';
            
            container.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div class="glass rounded-xl p-6">
                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2">Score de Performance</p>
                        <p class="text-3xl font-bold text-${scoreColor}-600 dark:text-${scoreColor}-400">${performanceScore}</p>
                        <p class="text-xs text-gray-500 dark:text-zinc-500 mt-1">de 100</p>
                    </div>
                    <div class="glass rounded-xl p-6">
                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2">Tempo Médio</p>
                        <p class="text-3xl font-bold text-gray-900 dark:text-white">${(stats.avg_response_time || 0).toFixed(0)}</p>
                        <p class="text-xs text-gray-500 dark:text-zinc-500 mt-1">ms</p>
                    </div>
                    <div class="glass rounded-xl p-6">
                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2">Requisições Lentas</p>
                        <p class="text-3xl font-bold text-orange-600 dark:text-orange-400">${stats.slow_requests || 0}</p>
                        <p class="text-xs text-gray-500 dark:text-zinc-500 mt-1">de ${stats.total_requests || 0}</p>
                    </div>
                    <div class="glass rounded-xl p-6">
                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2">Memória Média</p>
                        <p class="text-3xl font-bold text-gray-900 dark:text-white">${(stats.avg_memory_usage || 0).toFixed(1)}</p>
                        <p class="text-xs text-gray-500 dark:text-zinc-500 mt-1">MB</p>
                    </div>
                </div>
                
                ${endpoints.length > 0 ? `
                    <div class="glass rounded-2xl p-6 mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Endpoints Mais Lentos</h3>
                        <div class="space-y-3">
                            ${endpoints.map(endpoint => {
                                const severityColors = { critical: 'red', high: 'orange', medium: 'amber', low: 'gray' };
                                const severityColor = severityColors[endpoint.severity] || 'gray';
                                return `
                                    <div class="p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <div class="flex items-center justify-between mb-2 flex-wrap gap-2">
                                            <div class="flex items-center gap-3">
                                                <span class="px-2 py-1 rounded text-xs bg-${severityColor}-500/20 text-${severityColor}-400 border border-${severityColor}-500/30 font-semibold">${endpoint.method}</span>
                                                <code class="text-sm font-mono text-gray-900 dark:text-white">${endpoint.endpoint}</code>
                                            </div>
                                            <span class="text-lg font-bold text-${severityColor}-600 dark:text-${severityColor}-400">${endpoint.avg_response_time}ms</span>
                                        </div>
                                        <div class="flex gap-4 text-xs text-gray-600 dark:text-zinc-400">
                                            <span>${endpoint.request_count} requisições</span>
                                            <span>${endpoint.slow_count} lentas (${endpoint.slow_ratio}%)</span>
                                            <span>Máx: ${endpoint.max_response_time}ms</span>
                                        </div>
                                    </div>
                                `;
                            }).join('')}
                ...
                ${recommendations.length > 0 ? `
                    <div class="glass rounded-2xl p-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Recomendações</h3>
                        <div class="space-y-3">
                            ${recommendations.map(rec => {
                                const severityColors = { high: 'red', medium: 'orange', low: 'amber' };
                                const severityColor = severityColors[rec.severity] || 'gray';
                                return `
                                    <div class="p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <div class="flex items-start gap-3 mb-2">
                                            <span class="px-2 py-1 rounded text-xs bg-${severityColor}-500/20 text-${severityColor}-400 border border-${severityColor}-500/30 font-semibold">${rec.severity === 'high' ? 'Alta' : rec.severity === 'medium' ? 'Média' : 'Baixa'}</span>
                                            <div class="flex-1">
                                                <p class="font-semibold text-gray-900 dark:text-white mb-1">${rec.title}</p>
                                                <p class="text-sm text-gray-600 dark:text-zinc-400 mb-2">${rec.description}</p>
                                                <p class="text-xs text-gray-500 dark:text-zinc-500"><strong>Sugestão:</strong> ${rec.suggestion}</p>
                                            </div>
                                        </div>
                                    </div>
                                `;
