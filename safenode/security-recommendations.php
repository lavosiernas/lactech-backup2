...
$pageTitle = 'Recomendações de Segurança';
...
<body class="h-full" x-data="{ sidebarOpen: false }">
    ...
    if (typeof lucide !== 'undefined') {
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden bg-white dark:bg-dark-950">
            ...
            <header class="min-h-20 bg-white/80 dark:bg-dark-900/50 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 px-4 md:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0">
                <div class="flex items-center gap-3 md:gap-6 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight break-words">Recomendações de Segurança</h2>
                        <?php if ($selectedSite): ?>
            ...
            <div class="flex-1 overflow-y-auto p-8">
                <div id="recommendations-content" class="space-y-6">
                    <div class="glass rounded-2xl p-12 text-center">
                        <i data-lucide="loader" class="w-12 h-12 mx-auto mb-4 animate-spin text-gray-500 dark:text-zinc-400"></i>
                        <p class="text-sm text-gray-700 dark:text-zinc-500">Carregando recomendações de segurança...</p>
                    </div>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        async function fetchRecommendations() {
            try {
                const response = await fetch('api/security-recommendations.php?timeframe=7d');
                if (!response.ok) {
                ...
                if (data.success) {
                    ...
                    console.log('Security Recommendations Data:', recommendationsData);
                ...
                document.getElementById('recommendations-content').innerHTML = `
                    <div class="glass rounded-2xl p-8 text-center">
                        ...
                        <button onclick="fetchRecommendations()" class="px-4 py-2 bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white rounded-xl hover:bg-gray-200 dark:hover:bg-white/20 transition-colors text-sm">
                            Tentar novamente
                        </button>
                    </div>
                `;
        ...
        function updateRecommendationsPage() {
            ...
            if (recommendations.length === 0) {
                container.innerHTML = `
                    <div class="table-card p-8 text-center">
                        ...
                        <p class="text-gray-900 dark:text-white font-bold mb-2">Nenhuma recomendação disponível</p>
                        <p class="text-gray-600 dark:text-zinc-500 text-sm">Não foram detectados padrões de ataque que exigem recomendações específicas</p>
                    </div>
                `;
                lucide.createIcons();
                return;
            ...
            const severityColors = { critical: 'red', high: 'red', medium: 'orange', low: 'amber' };
            
            container.innerHTML = `
                <div class="glass rounded-2xl p-6 mb-6">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div>
                            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">${stats.total_recommendations || recommendations.length} Recomendaç${recommendations.length !== 1 ? 'ões' : 'ão'}</h2>
                            <p class="text-sm text-gray-600 dark:text-zinc-500 mt-1">Últimos 7 dias</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ${recommendations.map(rec => {
                        ...
                        return `
                            <div class="glass rounded-xl p-6 border-l-4 border-${color}-500">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2 flex-wrap">
                                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">${rec.title}</h3>
                                            <span class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-${color}-500/20 text-${color}-400 border border-${color}-500/30">
                                                ${severityLabels[severity]}
                                            </span>
                                        </div>
                                        <p class="text-sm text-gray-600 dark:text-zinc-400 mb-3">${rec.description}</p>
                                        
                                        <div class="mt-4 p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
                                            <p class="text-xs text-blue-400 font-semibold mb-2">RECOMENDAÇÃO:</p>
                                            <p class="text-sm text-gray-900 dark:text-white">${rec.suggestion}</p>
                                        </div>
                                        
                                        ${rec.affected_endpoints && rec.affected_endpoints.length > 0 ? `
                                            <div class="mt-4 pt-4 border-t border-gray-200 dark:border-white/10">
                                                <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2 font-semibold">Endpoints Afetados:</p>
                                                <div class="flex flex-wrap gap-2">
                                                    ${rec.affected_endpoints.slice(0, 5).map(endpoint => `
                                                        <span class="px-2 py-1 rounded text-xs bg-gray-500/20 text-gray-400 border border-gray-500/30 font-mono">
                                                            ${endpoint}
                                                        </span>
                                                    `).join('')}
                                                </div>
                                        ...
                                        <div class="mt-4 flex items-center gap-4 text-xs text-gray-600 dark:text-zinc-400">
                                            <span>Tentativas: <strong class="text-gray-900 dark:text-white">${rec.attack_count || 0}</strong></span>
                                            <span>Impacto: <strong class="text-${color}-600 dark:text-${color}-400">${rec.impact || 'N/A'}</strong></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
