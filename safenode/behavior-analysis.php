...
$pageTitle = 'Análise de Comportamento';
...
<body class="h-full" x-data="{ sidebarOpen: false }">
            ...
            <header class="min-h-20 bg-white/80 dark:bg-dark-900/50 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 px-4 md:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0">
                <div class="flex items-center gap-3 md:gap-6 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight break-words">Análise de Comportamento</h2>
                        <?php if ($selectedSite): ?>
            ...
            <div class="flex-1 overflow-y-auto p-8">
                <div id="behavior-content" class="space-y-6">
                    <div class="glass rounded-2xl p-12 text-center">
                        ...
                        <p class="text-sm text-gray-700 dark:text-zinc-500">Carregando análise de comportamento...</p>
                    </div>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        async function fetchBehaviorAnalysis() {
            try {
                const response = await fetch('api/behavior-analysis.php?timeframe=24h');
                if (!response.ok) {
                ...
                console.error('Erro ao buscar análise de comportamento:', error);
                document.getElementById('behavior-content').innerHTML = `
                    <div class="glass rounded-2xl p-8 text-center">
                        ...
                        <button onclick="fetchBehaviorAnalysis()" class="px-4 py-2 bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white rounded-xl hover:bg-gray-200 dark:hover:bg-white/20 transition-colors text-sm">
                            Tentar novamente
                        </button>
                    </div>
                `;
            ...
            if (anomalies.length === 0) {
                container.innerHTML = `
                    <div class="table-card p-8 text-center">
                        ...
                        <p class="text-gray-900 dark:text-white font-bold mb-2">Nenhuma anomalia detectada</p>
                        <p class="text-gray-600 dark:text-zinc-500 text-sm">Não foram detectados comportamentos suspeitos no período</p>
                    </div>
                `;
                lucide.createIcons();
                return;
            ...
            container.innerHTML = `
                <div class="glass rounded-2xl p-6 mb-6">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div>
                            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">${stats.total_anomalies || anomalies.length} Anomalia${anomalies.length !== 1 ? 's' : ''} Detectada${anomalies.length !== 1 ? 's' : ''}</h2>
                            <p class="text-sm text-gray-600 dark:text-zinc-500 mt-1">IPs com comportamento suspeito</p>
                        </div>
                    </div>
                </div>
                        ...
                        return `
                            <div class="glass rounded-xl p-6">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2 flex-wrap">
                                            <p class="text-lg font-mono font-bold text-gray-900 dark:text-white">${anomaly.ip_address}</p>
                                            <span class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-${severityColor}-500/20 text-${severityColor}-400 border border-${severityColor}-500/30">
                                                ${severityLabels[severity]} (${riskScore}%)
                                            </span>
                                            ${anomaly.country ? `<span class="text-xs text-gray-600 dark:text-zinc-400">${anomaly.country}</span>` : ''}
                                        </div>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                    <div class="p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">Requisições</p>
                                        <p class="text-lg font-bold text-gray-900 dark:text-white">${anomaly.stats?.total_requests || 0}</p>
                                    </div>
                                    <div class="p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">Endpoints</p>
                                        <p class="text-lg font-bold text-orange-600 dark:text-orange-400">${anomaly.stats?.unique_endpoints || 0}</p>
                                    </div>
                                    <div class="p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">Bloqueios</p>
                                        <p class="text-lg font-bold text-red-600 dark:text-red-400">${anomaly.stats?.blocked_count || 0}</p>
                                    </div>
                                    <div class="p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">Req/hora</p>
                                        <p class="text-lg font-bold text-gray-900 dark:text-white">${anomaly.stats?.requests_per_hour?.toFixed(1) || 0}</p>
                                    </div>
                                </div>
                                
                                ${anomaly.endpoints_sample && anomaly.endpoints_sample.length > 0 ? `
                                    <div class="mt-4 pt-4 border-t border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-2 font-semibold">Endpoints Acessados</p>
                                        <div class="flex flex-wrap gap-2">
                                            ${anomaly.endpoints_sample.slice(0, 5).map(endpoint => `
                                                <span class="px-2 py-1 rounded text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 font-mono">
                                                    ${endpoint}
                                                </span>
                                            `).join('')}
                                            ${anomaly.endpoints_sample.length > 5 ? `<span class="text-xs text-gray-600 dark:text-zinc-400">+${anomaly.endpoints_sample.length - 5} mais</span>` : ''}
                                        </div>
                                    </div>
                                ` : ''}
                                
                                <div class="mt-4 pt-4 border-t border-gray-200 dark:border-white/10 flex justify-between text-xs text-gray-600 dark:text-zinc-400">
                                    <span>Primeira: ${new Date(anomaly.first_seen).toLocaleString('pt-BR')}</span>
                                    <span>Última: ${new Date(anomaly.last_seen).toLocaleString('pt-BR')}</span>
                                </div>
                            </div>
                        `;
                    }).join('')}
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', function() {
                ...
                setInterval(fetchBehaviorAnalysis, 30000);
            });
        } else {
