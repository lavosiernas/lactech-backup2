...
<body class="h-full" x-data="{ sidebarOpen: false }">
    ...
    if (!function_exists('getSafeNodeUrl')) {
        function getSafeNodeUrl($route, $siteId = null) {
            $routes = [
                ...
                'sites' => 'sites.php',
                'logs' => 'logs.php',
                'suspicious-ips' => 'suspicious-ips.php',
                'threat-analysis' => 'threat-analysis.php',
                'behavior-analysis' => 'behavior-analysis.php',
                'security-recommendations' => 'security-recommendations.php',
                'security-monitor' => 'security-monitor.php',
                'performance' => 'performance.php',
                'alerts' => 'alerts.php',
                'reports' => 'reports.php',
            ];
            
            return $routes[$route] ?? $route . '.php';
        }
    }
    ...
    <aside x-show="sidebarOpen"
        ...
        <nav class="flex-1 p-4 space-y-1 overflow-y-auto overflow-x-hidden">
            ...
            <a href="<?php echo getSafeNodeUrl('security-monitor'); ?>" class="nav-item <?php echo $currentPage == 'security-monitor' ? 'active' : ''; ?>" @click="sidebarOpen = false">
                <i data-lucide="radio" class="w-5 h-5 flex-shrink-0"></i>
                <span class="font-medium whitespace-nowrap">Monitoramento Tempo Real</span>
            </a>
    ...
    if (typeof lucide !== 'undefined') {
        ...
        <main class="flex-1 flex flex-col h-full overflow-hidden bg-white dark:bg-dark-950">
            ...
            <header class="min-h-20 bg-white/80 dark:bg-dark-900/50 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 px-4 md:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0">
                <div class="flex items-center gap-3 md:gap-6 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight break-words">Análise de Ameaças</h2>
                        <?php if ($selectedSite): ?>
            ...
            <div class="flex-1 overflow-y-auto p-8">
                <div id="threat-content" class="space-y-6">
                    <div class="glass rounded-2xl p-12 text-center">
                        <i data-lucide="loader" class="w-12 h-12 mx-auto mb-4 animate-spin text-gray-500 dark:text-zinc-400"></i>
                        <p class="text-sm text-gray-700 dark:text-zinc-500">Carregando ameaças detectadas...</p>
                    </div>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        async function fetchThreats() {
            try {
                const timeframe = '24h';
                const response = await fetch(`api/threat-detection.php?timeframe=${timeframe}`);
                if (!response.ok) {
                ...
                console.log('Threat Detection API Response:', data);
                ...
                document.getElementById('threat-content').innerHTML = `
                    <div class="glass rounded-2xl p-8 text-center">
                        ...
                        <button onclick="fetchThreats()" class="px-4 py-2 bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white rounded-xl hover:bg-gray-200 dark:hover:bg-white/20 transition-colors text-sm">
                            Tentar novamente
                        </button>
                    </div>
                `;
        ...
        function updateThreatPage() {
            ...
            if (threats.length === 0) {
                container.innerHTML = `
                    <div class="table-card p-8 text-center">
                        <i data-lucide="shield-check" class="w-12 h-12 mx-auto mb-4 text-green-400"></i>
                        <p class="text-gray-900 dark:text-white font-bold mb-2">Nenhuma ameaça detectada</p>
                        <p class="text-gray-600 dark:text-zinc-500 text-sm">Não foram detectadas tentativas de ataque no período selecionado</p>
                    </div>
                `;
                lucide.createIcons();
                return;
            ...
            const typeStats = Object.entries(byType).map(([type, count]) => ({
                type,
                count,
                label: getThreatTypeLabel(type),
                color: getThreatTypeColor(type)
            ...
            container.innerHTML = `
                <div class="glass rounded-2xl p-6 mb-6">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div>
                            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">${stats.total_threats || threats.length} Ameaça${threats.length !== 1 ? 's' : ''} Detectada${threats.length !== 1 ? 's' : ''}</h2>
                    ...
                    ${typeStats.length > 0 ? `
                        <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                            ${typeStats.map(stat => `
                                <div class="p-3 rounded-xl bg-${stat.color}-500/10 border border-${stat.color}-500/20">
                                    <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">${stat.label}</p>
                                    <p class="text-lg font-bold text-${stat.color}-600 dark:text-${stat.color}-400">${stat.count}</p>
                                </div>
                            `).join('')}
                        </div>
                    ` : ''}
                ...
                <div class="space-y-4">
                    ${threats.slice(0, 50).map(threat => {
                        const color = getThreatTypeColor(threat.type);
                        const severityColors = {
                            'high': 'red',
                            'medium': 'orange',
                            'low': 'amber'
                        };
                        const severityColor = severityColors[threat.severity] || 'gray';
                                ...
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2 flex-wrap">
                                            <span class="px-3 py-1 rounded-lg text-xs font-semibold bg-${color}-500/20 text-${color}-400 border border-${color}-500/30">
                                            ...
                                            <span class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-${severityColor}-500/20 text-${severityColor}-400 border border-${severityColor}-500/30">
                                                ${threat.severity === 'high' ? 'Alto' : threat.severity === 'medium' ? 'Médio' : 'Baixo'}
                                            </span>
                                            <p class="text-sm font-mono text-gray-600 dark:text-zinc-400">${threat.ip_address}</p>
                                        </div>
                                        <p class="text-sm text-gray-900 dark:text-white font-semibold mb-1">${threat.endpoint}</p>
                                        ${threat.reason ? `<p class="text-xs text-gray-600 dark:text-zinc-500">${threat.reason.substring(0, 100)}${threat.reason.length > 100 ? '...' : ''}</p>` : ''}
                                    </div>
                                </div>
                                
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                    <div class="p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">País</p>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white">${threat.country || 'N/A'}</p>
                                    </div>
                                    <div class="p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                                        <p class="text-xs text-gray-600 dark:text-zinc-400 mb-1">Data/Hora</p>
                                        <p class="text-sm font-bold text-gray-900 dark:text-white">${new Date(threat.timestamp).toLocaleString('pt-BR')}</p>
                                    </div>
                                </div>
                            </div>
                        `;
